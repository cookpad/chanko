  ...
  module Sample
    ...
    models do
      expand("ExpandedModel") do
        #expanded_model.unit(:sample).has_many_associations
        ...
        # expanded_model_instance.unit.new_method
        def new_method
        end

        #expanded_mode.unit.new_method
        class_methods do
...
All expanded method must be used in your code via unit proxy as below.
  user_instance.unit.expanded_method
...
provides association and named_scope and class method syntax.
  models do
    expand("ExpandedModel") do
       #expanded_model.unit.has_many_associations
       has_many :has_many_associations
       has_one :has_one_association
...
label with "unit.label()" syntax.
  User.find(:first, :include => [unit.label(:recipes)])
...
And can use helpers in views and controllers via unit proxy.
  function(:sample) { unit.hello }
...
you can check the status of an chanko. In almost all situations, context is controller.（deprecated ?)
  unit(:sample).active? #=> return boolean
